...
export function BrandMark({ className, compact }: BrandMarkProps) {
  ...
  return (
    <View className={`items-center ${className ?? ''}`}>
      <Text className={`font-semibold uppercase text-tatra-muted-foreground ${line}`}>tatra</Text>
      <Text className={`-mt-0.5 font-semibold uppercase text-tatra-muted ${line2}`}>banka</Text>
    </View>
  );
}
